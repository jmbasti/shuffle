...
<?php include("includes/header.php")?> <!--this is optional as the only purpose is to separate the files so that the main index page will not be crowded-->
	
<!-- CODE BELOW IS WHERE THE MAIN CONTENT GOES-->

<h1 class="pageHeadingBig">You might Also Like</h1>

<div class="gridViewContainer">
<?php
		$albumQuery = mysqli_query($con, "SELECT * FROM album ORDER BY RAND() LIMIT 10"); /* this will select random and limit is only 10*/

		while($row = mysqli_fetch_array($albumQuery)) { /*this will loop to the album table */
			
			/* CODE below will output in the homepage the album artwork and album title as well as link to specic album id */
			/* CODE below is concatenated */
			...
			/*echo $row['title'] . "<br>"; . means to append/join together and br means to separate in a diff line (for testing)*/
			/*gridViewItem is created when we loop thru' the album table*/
		}
?>
</div>
...
<?php include("includes/footer.php")?> <!--this is optional as the only purpose is to separate the files so that the main index page will not be crowded-->
...
/*-*/
	...
	if you wish to again, just click on the insert tab in the phpmyadmin as per required table

14. continue with index.php user interface
15. create separate files to tidy up the index.php (header.php, footer.php. album.php)
16. insert via includes the header and footer.php to the new album.php
17. create an Artist.php 
18. create an Album.php 
